...
export default function Watchlist() {
  ...
  const { data: cryptocurrencies, isLoading, error } = useGetCryptocurrenciesQuery();
  const [filteredCoins, setFilteredCoins] = useState<CryptoCurrency[]>([]); // Define the correct type here

  useEffect(() => {
    if (cryptocurrencies) {
      const coins = cryptocurrencies.filter((coin: CryptoCurrency) => watchlist.includes(coin.id));
      setFilteredCoins(coins); // Set state with correct type
    }
